...
import { parseAdminSupplierFilters } from '../admin/suppliers/list';
import { validateCreate, validateUpdate } from '../admin/suppliers/write';
...
describe('admin suppliers handler gates', () => {
  it('list returns 405 for non-GET', async () => {
    const mod = await import('../admin/suppliers/list');
  ...
  it('write returns 405 for GET', async () => {
    const mod = await import('../admin/suppliers/write');
    const handler = mod.default;
    let status = 0;
    let ended = false;
    const res = {
