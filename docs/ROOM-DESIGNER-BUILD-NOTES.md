...
- Choose **Stairs** and tap clear space on either of two adjacent floors. The same footprint must fit both rooms. The upper slab gains an opening. Build details offer width, run, X/Y, rotation and deletion; stairs also appear on both plan floors.
...
Client/API typechecks, changed-file lint and the production build pass. The complete local suite ran 2,665 tests: one contact-shadow regression was fixed and its affected suites passed on rerun. GitHub's typecheck and complete Vitest checks pass for the building commit. Backend requests are covered with mocks; no live customer records were written during testing.

Local phone visual checks confirmed two stacked storeys and a flat felt roof. Further localhost browser actions were blocked by automatic approval review. Deployment-specific browser/HTTP verification is recorded in BUILD-LINKS.md, rather than inferred from a successful build.
