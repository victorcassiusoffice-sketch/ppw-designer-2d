...
- Furnished show home: https://ppw-designer-2d-9dar1nmsw-victor-ppw.vercel.app/designer?demo=courts
- Plan has directly accessible rounded controls for Floors, Roof, Solar, 3D, Plot, Snap and Grid; phone wraps in full-width cells. Plot/Snap/Floors/room dimensions use reserved header space, Close/Escape/click-away and focus return. The badge reads Demo, preserving supplier attribution.
- The 3D floor selector offers Add floor and Roof. New floors copy layout/openings/finishes/free walls without furniture; from Roof, the top storey is copied. Door/Window/Stairs are direct actions. Existing validation and shared Plan/3D data remain.
- Roof keeps the whole building visible, with flat/gable/shed profiles and felt/tile/metal. Panels follow the same visible roof surface used for picking and movement. Ridge-spanning panels have labelled raised level preview mounts. Roof exit hides the covering; first mobile solar drop waits for surface readiness. Jinko has an original dark 108-cell/frame dimensional preview using exact catalog dimensions, with disclosure in item and cost details.
- The checkout-package icon and persistent estimate open Products & cost. Product selection shows the real catalog photo/supplier/price/dimensions/notes; the panel is beside the desktop scene or below the phone scene and closes via Close/Escape/click-away. Existing cart quantities/totals and checkout remain; structure/labour/delivery are excluded.
- Fixed roof identity loss in saved/load-normalized rooms, roof rotation incorrectly using indoor wall clearance, and duplicated roof slabs after adding an exact-footprint floor. Existing rooftop PV/surface work transfers to the new top slab; changed or ambiguous footprints preserve orphan work safely.
...
- Main-pass live QA at `b4softmgz` added a First floor, placed/rotated Jinko on a gable roof, added connecting stairs/window/door, and verified all in Plan with shared totals and energy inputs. Standard/TintEX/Courts loaded on desktop and 360/390px phones. Details, catalog, Snap/Plot, Close/Escape/click-away and no horizontal overflow/console errors were checked. Final polish corrects phone Grid width, long total clipping, automatic roof settings and washed-out Jinko cells.
- Local production build, client/API typechecks and changed-file lint passed. Main CI passed 250 files / 2,826 tests; final focused polish checks passed 5 files / 33 tests. Final CI/live results are above. No live customer cloud save, quote request or purchase was submitted.
- Workflow is saved here and in outputs/Room-Designer-Workflow.md. BUILD-LINKS, Desktop CURRENT-WORK-LINKS, both original handoff copies, PR #36 and saveable build notes are synchronized in the final documentation commit. Continue from this checkpoint, not the historical links below.

Remaining larger direction: exact manufacturer models/photorealism, direct editing of existing 3D room/wall edges, joined roofs, freeform terrain and atomic whole-building translation. Existing Move view pans the camera. Roof mounts remain planning previews; solar calculation tilt stays an explicit input. No game code/assets or fake supplier API were added.
...
### Historical ownership / continuation details — previous pass
...
2. Use the verified links in the newest Resume here checkpoint above. This floor/roof/cost pass is shipped; TintEX remains the paint demonstration and Courts the furnished show home.
...
4. Remaining Sims/building backlog: editing/moving existing wall and room edges directly in 3D, joined roofs, freeform terrain, and an atomic whole-building move if added. Connected arbitrary 3D wall runs and validated closed-room creation are now implemented, alongside rectangular room drag. Existing Move view is camera panning; it is not whole-building translation. Floors/stairs/openings/garden/pan/item movement/solar paths already work and must be preserved.
