...
const T = [
  ...
  ['p1-capture-flow-finish','Merchant capture','capture/* FSM','Finish capture flow (side/back shots, error states, reconcile UX)','DONE on feat/designer-capture-flow-2026-06-09 (precondition p0-capture-pdf-500 re-verified 11/11). Pure FSM module (captureFsm.ts: steps, shot-set front/side/back, submit gating, error catalogue) + first-class side/back ShotSet step + camera permission/PDF-fetch/blur/upload recover states (no dead ends) + reconcile review (all shots, retake, gated Submit, side/back ride into packet photoSide/photoBack — schema already supported). 18 new tests (12 FSM + 6 render). build+lint clean, 1411/1411 suite, render-verified, 12/12 fns held, no api/schema change. Real-device front/side/back tap + merge/deploy = GATE-2.','done','P1','code','src/lib/capture/captureFsm.ts; src/components/capture/CaptureModal.tsx; src/components/capture/CameraStage.tsx; src/components/capture/ShotSet.tsx; src/components/capture/ReviewSubmit.tsx',null,'Shipped feat/designer-capture-flow-2026-06-09; merge+deploy is GATE-2 (Vic).'],
