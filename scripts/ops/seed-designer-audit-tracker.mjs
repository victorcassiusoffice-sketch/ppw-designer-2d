...
const T = [
  // ---- P0 ----
  ['p0-capture-pdf-500','Merchant capture','merchants-router /api/capture/reference-page.pdf(+v2)','Fix capture reference PDF 500','Both reference-page.pdf and -v2.pdf return HTTP 500 FUNCTION_INVOCATION_FAILED in prod. Merchant cannot print calibration sheet -> whole phone-capture onboarding broken. Debug jsPDF/autotable serverless bundling/font/buffer issue.','todo','P0','live+code','api/merchants-router.ts; api/_lib/capture/referencePage.ts',null,'Confirmed live 2026-06-03 curl. Not visible in Sentry (server errors uncaptured).'],
  ...
  ['p1-stripe-connect-webhook','Merchant payouts','stripe-connect/webhook','Flesh out Stripe Connect payout webhook','account.updated -> merchant KYC status. Currently a thin stub. Needed before any merchant payout run.','todo','P1','code','api/stripe-connect/webhook.ts; api/_lib/stripeConnect.ts; api/_lib/stripeConnectWebhook.ts',null,'Gated by STRIPE_MU_SUPPORTED env.'],
  ...
  ['p2-stripe-dedupe-kv','Payments','stripe-webhook','Move Stripe webhook dedupe to KV','In-memory Set is single-lambda only. Move to Upstash for multi-instance idempotency.','todo','P2','code','api/stripe-webhook.ts; api/_lib/webhookDedupe.ts',null,null],
  ['p2-migration-gap','DB','Neon migrations','Document migration gap 0012-0023','Migrations jump 0011 -> 0024. Add a note so it does not read as missing.','todo','P2','code','api/_db/migrations/',null,null],
  // ---- Sentry / observability ----
  ['sentry-server-errors-uncaptured','Observability','Sentry javascript-react project','Server-side errors not captured in Sentry','javascript-react project shows 0 events received in 30d and the live PDF 500 did not surface. @sentry/node likely not reporting serverless errors to a project. Wire server error capture so prod 500s are observable.','todo','P1','sentry','api/_lib/sentry.ts; api/healthcheck.ts',null,'3 issue-alert rules exist (New issue ppw-designer-2d; high-priority; Email #ppwellness) but no events flowing.'],
  ...
  ['p3-paint-flooring-ui','Customer','/api/calc/:type','Paint/flooring calculator customer UI','Calc API folded into merchants-router; customer UI partial. Pending product decision.','todo','P3','code','api/_lib/calc/paintCalcHandler.ts; src/lib/paintCalculator.ts',null,null],
  ['p3-neon-isolation','Infra','Neon DB','Neon prod/preview DB isolation','Single branch -> previews + Dependabot PRs touch prod data. Consider a preview branch before external merchant load.','todo','P3','brain','api/_db/client.ts',null,'Per api-deploy-topology.md risk note.'],
  ['p3-agent-costcap','Merchant agent','agent-chat','Agent per-merchant cost-cap enforcement (Phase 6 pt2)','OpenRouter live; merchant auth + per-merchant cost cap not yet enforced.','todo','P3','code','api/agent-chat.ts; api/_lib/agent/lockdown.ts; api/_lib/agent/openrouter.ts',null,null],
  ...
  ['ok-cart-quote','Backend','cart-quote','Cart split quote','Live; per-merchant split.','done','-','live','api/cart-quote.ts; api/_lib/cart/split.ts',null,null],
  ...
  ['ok-admin-router','Admin','admin-router','Admin console + auth gate','401 on unauth (Clerk gate works); list/detail/approve/reject/products/suppliers/stats/audit.','done','-','live+code','api/admin-router.ts; api/_lib/admin/*; api/_lib/adminAuth.ts',null,null],
  ...
  ['ok-cron','Backend','cron-router','Cron jobs','escalate-orders (9am), refresh-supplier-rating, email-reconcile; CRON_SECRET-gated.','done','-','code','api/cron-router.ts; api/_lib/cron/*',null,null],
  ['ok-merchant-auth','Merchant','merchantSession','Magic-link HMAC sign-in','30-day HMAC token; RequireMerchant guard.','done','-','code','api/_lib/merchantSession.ts; src/components/RequireMerchant.tsx',null,null],
